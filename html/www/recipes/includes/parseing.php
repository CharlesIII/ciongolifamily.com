<?php
    ...
    foreach ($text as $key => $line) {
        ...
        if ($line != "") {
            ...
            if (preg_match("/[0-9]+/", $line)==0 && strpos($line,',')===false) { //this line contains no commas or numbers it may be a header
            ...
            //need to find text between brackets containing numbers
               ...
               if ( $dash>0 || $to>0) { //we may have a range in the quantity
                    if ($dash>0) {
                        //check the dash is between numers to ensure it is quantity range
                        $first = substr($ingpart,0,$dash);
                        $lastinfirst = explode(" ", trim($first));
                        $numwords = (count($lastinfirst) - 1);
                        $lastwinfirst = $lastinfirst[$numwords];
                        $second = substr($ingpart,$dash + 1);
                        $firstinsecond = explode(" ", trim($second));
                        $firstwinsecond = $firstinsecond[0];
                        $isfirstnum=extract_numbers($lastwinfirst);
                        $issecondnum=extract_numbers($firstwinsecond);
               ...
               if (isset($match) && isset($match[0])) {  //we have a qty
                    $ct=-1;
                ...
                foreach ($mwunitarray as $uvalue) {
                 ...
                 if (sizeof($ingarrayp) > 1) {
                     ...
                     ${'ing'.$key}=trim(${'ing'.$key});
                 
                }  else {
                    ${'ing'.$key} = $ingarrayp[0];
                }
                $scolon=substr_count($line, ";", 0);
                if ( $scolon > 0 || isset($extraqtymsg)) { //we have prepreps
                    unset($pparray);
